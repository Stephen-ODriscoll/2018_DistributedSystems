...
public class Local implements Monitor {
	...
	public synchronized void checkChanges(ArrayList<byte[]> newFiles, ArrayList<String> newFileNames) {
		
		ArrayList<File> localCopy = new ArrayList<File>();		//I make a copy of my files
		ArrayList<File> sharedCopy = new ArrayList<File>();
		File folder = new File(myDirectory + "\\TempShared");
		folder.mkdir();
		...
		//I save shared files to temporary folder
		for(int i = 0; i < newFiles.size(); i++) {
			
			try {
				File temp = new File(myDirectory + "\\TempShared\\" + newFileNames.get(i));
				Files.write(temp.toPath(), newFiles.get(i));
				
			} catch (IOException e) {

				System.out.println("Error Writing Shared Files to Folder: " + e);
			}
		}
		
		File[] filesArray = folder.listFiles();		//Take in these files as an array

		//Add files in this array to my ArrayList
		for (int i = 0; i < filesArray.length; i++)
			sharedCopy.add(filesArray[i]);
		
		System.out.println("Shared: " + sharedCopy.size() + " Local: " + localCopy.size());
		
		//I check all files in both lists against each other
		for(int a = 0; a < sharedCopy.size(); a++)
			for(int b = 0; b < localCopy.size(); b++) {
				
				//If two files are the exact same content
				try {
					if(FileUtils.contentEquals(sharedCopy.get(a), localCopy.get(b))) {
						...
						sharedCopy.remove(a);		//Remove the file from shared copy
					...
					else if(sharedCopy.get(a).getName().equals( localCopy.get(b).getName() )) {
					...
					System.out.println("Error Checking Local Against Shared");
				}
				
				//If removing a file from a means there are no more
				if(a >= sharedCopy.size())
					break;					//Stop looping
			}
			...
			for(int i = 0; i < sharedCopy.size(); i++) {
				
				gui.changeEntry(sharedCopy.get(i).getName(), "red");		//Red means file is in shared but not local
				differences.remove(sharedCopy.get(i).getName());			//If this file was different it isn't now
			}
	...
	public synchronized void addFiles(ArrayList<String> names, ArrayList<byte[]> add) {
		...
		for(; a < add.size(); a++) {
			...
			for(int i = 0; i < files.size(); i++)
				try {
					if(Arrays.equals(Files.readAllBytes(files.get(i).toPath()), add.get(a)))
						files.remove(i);
				} catch (IOException e) {
					
					System.out.println("Failed to Write File to Local");
				}
		
			String name = names.get(a);		//Get the name of this file
		
			try {
			
				Files.write( new File(myDirectory + "\\" + name).toPath(), add.get(a));				//Try to copy this file to local folder
		...
		for(int a = 0; a < names.size(); a++)
			for(int b = 0; b < files.size(); b++)
				if(files.get(b).getName().equals(names.get(a)))
					try {
						Files.delete(files.get(b).toPath());			//Try to delete this file
